#automatic segmentation of lateral ventricles
...
#global variables that speed up execution of program during parameter tuning phase
...
#Processing inputs for the user-interface. Calls main processing function with appropriate items from webpage.
...
def submit_inputs():
    ...
    # call main function and return 3D plot renderinng
    return return_3D_image(DICOM_FOLDER_PATH, SERIES_UID, SPACING, LARGEST_REGION_INDEX, ERODE_DILATE_ITER,
                    MID_PUPILLARY_Z_INDEX, RIGHT, OFFSET, SENS_VENT, ROT_UP_DOWN, ROT_LEFT_RIGHT, SHIFT_LEFT_RIGHT)


#Renders empty fields template for beginning
@app.route('/')
def make_home():
    return render_template('index.html', graphJSON='')

#Main function that processes input images, applies parameters, and returns 3D image of skull, segmented ventricle,
# and EVD path.
def return_3D_image(DICOM_FOLDER_PATH, SERIES_UID, SPACING, LARGEST_REGION_INDEX, ERODE_DILATE_ITER,
    ...
    if resampled_arr is None or stored_dicom_ct_path != DICOM_FOLDER_PATH or stored_seriesuid != SERIES_UID:
        ...
        #If multiple volumes in one folder, select which one to use (user-specified)
        if SERIES_UID:
            arr = []
            for aslice in slices:
        ...
        #Identify connected regions and isolate the 1 or 2 largest ones to designate as ventricles
        ...
        #Saving into global variables for faster execution during tuning
        ...
        #Loading in data from global variables
    ...
    #Starting from the nasion, going cranially 11cm
    ...
    #Loop until we have moved 11cm cranially
    ...
    # Loop until we have moved 3cm right or left
    while cur_dist < fin_distance:
        cur_index -= 1
        edge_points = get_edgepoints(brain_skull_mask[cur_index, :, :])
        if RIGHT:
    ...
    #Calculate target location of EVD in ventricles
    ...
    #Isolate front half of ventricle
    ...
    #Move forwards until we get to an appropriate location
    ...
    #Apply manual left/right target adjustment
    z_val = sag_midpt - SHIFT_LEFT_RIGHT


    #Proposed EVD target point in ventricle
    ...
    #Draw points for nasion, kocher, and EVD target
    ...
    #To save as html
    #fig_save = plot(fig, filename='%s_%0.2f_%d_both_mask.html' % (
                 #DICOM_FOLDER_PATH.split('/')[-1], SPACING, SENS_VENT),
                            #auto_open=False)
...
def load_scan(path):
    ...
    # Try using imagePositionPatient attribute if available, otherwise use InstanceNumber
    try:
    ...
    # Try using ImagePositionPatient for slice thickness, otherwise use SliceLocation
    try:
...
def calc_offset_ventricle_pt(kocher_point, ventricle_point, ventricle_mask, rerun=False):
    ...
    #While distance from proposed point is <30 pixels from current point being tested and we have not passed
    # through all ventricle pixels, check if this trajectory is valid
    while offset < 30 and try_another_offset:
        ...
        #Go through this test EVD trajectory to see if it passes through ventricle and terminates
    ...
    #If no appopriate trajctory is found (i.e EVD keeps piercing into and exits ventricle), rerun and set EVD target
    # as the bottom of the ventricle
    if offset == 30:
        if not rerun:
...
#Function for standardizing the spacing between slices to 1x1x1mm for manually created masks
...
#From a binary mask of one axial slice, returns x and y coordinates of all edge points
